...
Excel is a much used tool in the biochemical laboratory, both to record and to 
analyze experiments. Often, to save work, we create excel templates when these
experiments become a routine. When analyzing a lot of these experiments 
switching to a scripting language like R or Python for analysis will become 
useful. Also in these cases the excel template is a useful way to structure the
recording of experimental data and metadata.

The goal of the excelDataGuide package is to be able to use both spreadsheet 
and scripts as data analysis tools. 
...
Preferably, it also has a name.

#### Version rules

Note that template versions must follow the R package versioning rules 
otherwise checking version numbers will not work. This implies that 
...
Hence a version like <kbd>2</kbd> is invalid, but <kbd>2.0</kbd> is OK.
